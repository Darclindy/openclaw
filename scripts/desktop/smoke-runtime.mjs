...
async function waitForConnectChallenge(ws, timeoutMs = 5000) {
  return await new Promise((resolve) => {
    ...
    const onMessage = (data) => {
      const parsed = JSON.parse(data.toString());
      const nonce = parsed?.type === "event" && parsed.event === "connect.challenge"
  ...
  try {
    ...
    try {
      ...
      if (!history.ok || !Array.isArray(history.payload?.messages)) {
        fail(`chat.history did not return messages for created session: ${JSON.stringify(history)}`);
      }
    } finally {
      ws.close();
    }
...
if (launcherText.includes('"--auth",\n      "none"') || launcherText.includes("'--auth',\n      'none'")) {
