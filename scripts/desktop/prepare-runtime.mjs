...
function assertUsableNode(nodeBinary) {
  ...
  if (!Number.isInteger(major) || major < 22) {
    throw new Error(`OpenClaw desktop sidecar requires Node 22 or newer; got ${version}`);
  }
  return version;
}
...
function main() {
  ...
  if (
    ...
    throw new Error(
      "Desktop release packaging requires OPENCLAW_DESKTOP_OPENCLAW_RUNTIME_DIR or an existing resources/openclaw-runtime/openclaw runtime. Set OPENCLAW_DESKTOP_ALLOW_DEV_RUNTIME_FALLBACK=1 only for local shell testing.",
    );
  }

  const hasLobster = maybeCopyLobsterArtifact();
  if (process.env.OPENCLAW_DESKTOP_WRITE_RUNTIME_MANIFEST === "1") {
  ...
  if (!hasOpenClawRuntime) {
    console.log("[desktop] no packaged OpenClaw runtime configured; development checkout fallback remains enabled");
  }
}
