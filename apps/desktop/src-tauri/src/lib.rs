...
fn desktop_start_gateway(
    ...
    let requested_port = port.unwrap_or(DEFAULT_GATEWAY_PORT);
    {
        ...
        if let Some(process) = gateway.as_ref() {
            if process.port == requested_port {
    ...
    let port = resolve_available_gateway_port(requested_port)?;
...
fn desktop_restart_gateway(
    ...
    let auth_token = get_or_create_gateway_auth_token(state.inner())?;
    let selected_port = resolve_available_gateway_port(port.unwrap_or(DEFAULT_GATEWAY_PORT))?;
    spawn_gateway(app, selected_port, auth_token)
}
            ...
            tauri::async_runtime::spawn(async move {
                ...
                let result =
                    get_or_create_gateway_auth_token(state.inner()).and_then(|auth_token| {
                        spawn_gateway(handle.clone(), DEFAULT_GATEWAY_PORT, auth_token)
                    });
