...
function parseToken(argv) {
  ...
  const raw = tokenIndex === -1 ? process.env.OPENCLAW_GATEWAY_TOKEN : argv[tokenIndex + 1];
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("exit", (code, signal) => {
      if (code === 0) {
        ...
        writeFileSync(
          markerPath,
          `${JSON.stringify({ installedAt: new Date().toISOString(), artifact }, null, 2)}\n`,
        );
        resolve();
        return;
      }
...
async function main() {
  ...
  await maybeInstallBundledLobster(openclaw, env);
