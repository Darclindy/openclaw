import { setControlUiPerfLogSink } from "./control-ui-performance.ts";
 ...
 */
function installDesktopPerfLogSink(): boolean {
  const tauri = getTauri();
  if (!tauri?.core?.invoke) {
    return false;
  }
  const invoke = tauri.core.invoke;
  setControlUiPerfLogSink((entry) => {
    void invoke("desktop_append_perf_log", { line: JSON.stringify(entry) }).catch(() => undefined);
  });
  return true;
}

export function initNativeBridge(host: NativeBridgeHost): () => void {
  const perfSinkInstalled = installDesktopPerfLogSink();
  const teardownPerfSink = () => {
    if (perfSinkInstalled) {
      setControlUiPerfLogSink(null);
    }
  };
