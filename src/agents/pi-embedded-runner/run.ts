...
import { traceAgentSpan } from "../agent-timeline.js";
import {
...
export async function runEmbeddedPiAgent(
  ...
  return enqueueSession(() => {
    ...
    return enqueueGlobal(() =>
      // Root agent-layer span for the perf-trace flame graph (runId matches the
      // gateway chat_send span so gateway/agent/provider lanes group together).
      // withCpuProfile captures a V8 profile of this run when OPENCLAW_CPU_PROFILE_DIR
      // is set (no-op otherwise). The body is reindented by the formatter; the
      // functional change is just this wrapper (see `git diff -w`).
      traceAgentSpan(
        "agent.run",
        () =>
          withCpuProfile(`agent-run-${params.runId}`, async () => {
            throwIfAborted();
            ...
            const notifyExecutionPhase = (
              phase: Parameters<
                NonNullable<RunEmbeddedPiAgentParams["onExecutionPhase"]>
              >[0]["phase"],
              extra?: Omit<
            ...
            if (lockedProfileId) {
              if (pluginHarnessOwnsTransport) {
                ...
                if (
                  !lockedProfile ||
                  !lockedProfileProvider ||
                  lockedProfileProvider !== runProvider
                ) {
                  lockedProfileId = undefined;
                }
              }
              ...
              while (true) {
                  ...
                  log.info(
                    `[context-overflow-precheck] early recovery route=${preflightRecovery.route} ` +
                      ...
                      (retryingFromTranscript
                        ? "retrying from current transcript"
                        : "retrying prompt"),
                  );
                  ...
                  if (timeoutCompactionAttempts >= MAX_TIMEOUT_COMPACTION_ATTEMPTS) {
                    ...
                    if (timeoutCompactResult.compacted) {
                      ...
                      if (
                        ...
                        lastCompactionTokensAfter = Math.floor(
                          timeoutCompactResult.result.tokensAfter,
                        );
                      }
                ...
                if (promptError && !aborted && promptErrorSource !== "compaction") {
                  ...
                  const promptFailoverFailure =
                    ...
                    isFailoverErrorMessage(errorText, { provider });
                ...
                if (
                  ...
                  const replayInvalid =
                    attempt.promptTimeoutOutcome?.replayInvalid ??
                ...
                if (
                  ...
                  !timedOut &&
                  sessionLastAssistant?.stopReason === "error" &&
                  ((sessionLastAssistant?.usage as { output?: number } | undefined)?.output ??
                    0) === 0 &&
