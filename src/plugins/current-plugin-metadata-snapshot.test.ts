...
describe("current plugin metadata snapshot", () => {
  ...
  it("keeps derived registry snapshots as the current snapshot (process-stable)", () => {
    const derived = createSnapshot({ registrySource: "derived" });
    setCurrentPluginMetadataSnapshot(derived);

    // Derived snapshots are now reusable; lifecycle hooks clear the current
    // snapshot on install/reload so new plugins are still discovered.
